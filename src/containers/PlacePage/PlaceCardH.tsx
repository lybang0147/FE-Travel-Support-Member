...
export interface PlaceCardHProps {
  className?: string;
  userliked?: boolean;
  data: Place;
  distance?:  number;
...
const PlaceCardH: FC<PlaceCardHProps> = ({ className = "", data, userliked, distance }) => {
  const {
    name,
    addressDescription,
    minPrice,
      ...
      <div className="flex-grow p-3 sm:p-5 flex flex-col">
        ...
        <div className="flex justify-between items-end">
          ...
          <span className="text-base font-semibold text-secondary-500">
            {minPrice === maxPrice ? (
              <>
                {minPrice === 0 ? "Miễn phí" : `Giá tham khảo ${minPrice.toLocaleString("vn")} VND`}
              </>
            ) : (
              <>
                Giá tham khảo {`${minPrice.toLocaleString("vn")}-${maxPrice.toLocaleString("vn")}`} VND
              </>
            )}
          </span>
          {distance && (
  ...
  return (
    ...
    >
      <Link to={`/listing-place/place/${id}`} className="absolute inset-0"></Link>
